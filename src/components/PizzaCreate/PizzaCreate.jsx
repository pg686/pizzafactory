...
export default function PizzaCreate() {
    ...
    const onProductChange = (e) => {
        ...
        const category = Object.keys(products).find(cat => value in products[cat]);
        if (!category) {
        ...
        setDescription(prevDescription => checked
            ? [...prevDescription, value]
            : prevDescription.filter(item => item !== value)
        );

        // Update pizza type
        const allCheckedIngredients = Object.values(updatedProducts).flatMap(cat =>
            Object.values(cat).filter(item => item.checked)
        );

        if (allCheckedIngredients.some(item => item.type === 'meat')) {
            setPizzaType('meat');
        } else if (allCheckedIngredients.some(item => item.type === 'vegetarian')) {
