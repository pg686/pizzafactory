...
export const likeComment = async (email, commentId, isLiked) => {
  ...
  try {
    if (!isLiked) {
      ...
      result = true;
      const findDislike = await request.get(
        `${baseUrl}/commentDislikes?where=${queryCommentId}`,
      );
      if (!!findEntryByUser(findDislike, email)) {
        await request.remove(
          `${baseUrl}/commentDislikes/${findEntryByUser(findDislike, email)._id}`,
        );
      }
    } else {
      const getLikes = await request.get(
        `${baseUrl}/commentLikes?where=${queryCommentId}`,
      );
      if (!!findEntryByUser(getLikes, email)) {
        await request.remove(
          `${baseUrl}/commentLikes/${findEntryByUser(getLikes, email)._id}`,
        );
        result = false;
      }
    }
...
const findEntryByUser = (collection, email) =>
  collection?.find((x) => x.email === email);
export const dislikeComment = async (email, commentId, isDisliked) => {
  ...
  try {
    if (!isDisliked) {
      ...
      result = true;
      const findLike = await request.get(
        `${baseUrl}/commentLikes?where=${queryCommentId}`,
      );
      if (!!findEntryByUser(findLike, email)) {
        await request.remove(
          `${baseUrl}/commentLikes/${findEntryByUser(findLike, email)._id}`,
        );
      }
    } else {
      const findDislikes = await request.get(
        `${baseUrl}/commentDislikes?where=${queryCommentId}`,
      );
      if (!!findEntryByUser(findDislikes, email)) {
        await request.remove(
          `${baseUrl}/commentDislikes/${findEntryByUser(findDislikes, email)._id}`,
        );
        result = false;
      }
    }
